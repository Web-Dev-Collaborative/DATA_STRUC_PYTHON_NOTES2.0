    ...
    def medianSlidingWindow(self, nums: List[int], k: int) -> List[float]:
        ...
        for i, n in enumerate(nums[k:]):
            if n <= -maxHeap[0][0]:
                ...
                if nums[i] >= -maxHeap[0][0]: self.move(maxHeap, minHeap)
            else:
                heapq.heappush(minHeap, (n, i + k))
                if nums[i] <= minHeap[0][0]: self.move(minHeap, maxHeap)

            while maxHeap and maxHeap[0][1] <= i: heapq.heappop(maxHeap)
            while minHeap and minHeap[0][1] <= i: heapq.heappop(minHeap)
            if k & 1:
                result.append(-maxHeap[0][0])
            else:
                result.append((minHeap[0][0] - maxHeap[0][0]) / 2)
