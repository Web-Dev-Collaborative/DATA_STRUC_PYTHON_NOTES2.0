...
class AudioToText:
    ...
    def get_text(self):
        ...
        sound = AudioSegment.from_file(
            self.input, format=pathlib.Path(self.input).suffix.replace(".", "")
        )
            ...
            with sr.AudioFile(obj) as source:
                ...
                try:
                    text = self.rec.recognize_google(
                        audioListened, language=self.language
                    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        prog=os.path.basename(__file__), description=description
    )
    ...
    parser.add_argument(
        "-o", "--output", help="The output file for the text. Default 'outputText.txt'"
    )
    parser.add_argument(
        "-l",
        "--language",
        help="See 'https://cloud.google.com/speech-to-text/docs/languages' \
