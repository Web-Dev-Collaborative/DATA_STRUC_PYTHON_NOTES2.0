...
ratings = pd.read_csv('C:/Users/xxx/DataScience-Python3_/ml-100k/u.data',
                      sep='\t', names=r_cols, usecols=range(3), encoding="ISO-8859-1")

m_cols = ['movie_id', 'title', 'date']
movies = pd.read_csv('C:/Users/xxx/DataScience-Python3_/ml-100k/u.item',
                     sep='|', names=m_cols, usecols=range(3), encoding="ISO-8859-1")
...
for row in ratings.itertuples():
    # 1:user_id, 4:movie_id, 5:rating
    movieRatingsZ[row[1]-1, row[4]-1] = row[5]

movieRatings.shape
movieRatingsZ.shape  # has more columns?

# Let's determine the sparsity in the data
# 0 is here not rated / missing.
sparsity = float(len(movieRatingsZ.nonzero()[0]))
...
ratings_train, ratings_test = train_test_split(
    movieRatingsZ, test_size=0.33, random_state=42)
...
top_k_user_stats = top_k_user_ratings.groupby(
    'title').agg({'rating': [np.size, np.mean]})
top_k_user_stats.head()
'list(top_k_user_stats.columns.values)  # get the column names
df1 = df.assign(top_k_user_stats=df[(
    'rating', 'mean')] * top_k_user_stats[('rating', 'size')] / k)
# list(df1.columns.values) #get the column names
df2 = df1.sort_values([('top_k_user_stats', ''),
                       ('rating', 'mean'), ('rating', 'size')], ascending=False)
...
for i in range(ratings_train.shape[0]):
    user_pred_k[i, :] = top_k_distances[i].T.dot(
        ratings_train[top_k_users][i]) / np.array([np.abs(top_k_distances[i].T).sum(axis=0)]).T
