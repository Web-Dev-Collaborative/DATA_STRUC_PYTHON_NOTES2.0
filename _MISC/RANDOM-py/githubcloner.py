...
class getReposURLs:
    ...
    def fromOrgIncludeUsers(
        self, org_name, username=None, token=None, include_gists=False
    ):
        """
        Retrieves a list of repositories for a Github organization
        ...
        for user in users:
            URLs.extend(
                getReposURLs(api_prefix, exclude_repos).fromUser(
                    user, username=username, token=token, include_gists=include_gists
                )
            )
            ...
            if include_organization_members is False:
                URLs.extend(
                    getReposURLs(api_prefix, exclude_repos).fromOrg(
                        organization, username=username, token=token
                    )
                )
            else:
                URLs.extend(
