...
def main(argv=[], path="/tmp", outfile="test.html", title="Test", use_jquery=False):
    ...
    printc(
        "<magenta>The output file will be <white>'<u>{fullpath}<U>'.".format(
            fullpath=fullpath
        )
    )
...
<div id="content" class="container" style="font-size:140%;">"""
    ...
    <div class="alert alert-dismissable alert-warning">
        ...
        for inputfile in argv:
            try:
                printc(
                    "# <INFO> Trying to read from the file '<green><u>{inputfile}<reset><white>'.".format(
                        inputfile=inputfile
                    )
                )
                with open(inputfile, "r") as openinputfile:
                    printc(
                        " I opened it, to '{openinputfile}'.".format(
                    ...
                    try:
                        ...
                        try:
                            ...
                            html = BeautifulSoup(
                                markdown_text,
                                "html.parser",
                                parse_only=only_xmp_tag,
                                from_encoding="utf-8",
                            )
                            if beta:
                                print(
                                    " BTW, this html read with Beautiful soup has the encoding,",
                            ...
                            markdown_text = markdown_text.replace(
                                "```python",
                                '<pre><code class="language-python" style="font-size:145%;">',
                            )
                            markdown_text = markdown_text.replace(
                                "```bash",
                                '<pre><code class="language-bash" style="font-size:145%;">',
                            )
                            markdown_text = markdown_text.replace(
                                "```", "</code></pre>"
                            )
                            ...
                            printc(
                                " <warning> Exception found: <yellow>{e}<white>.".format(
                                    e=e
                                )
                            )
                            printc(
                    ...
                    printc(
                        " <blue>I wrote this to the output file '{html_file}'<white>.".format(
                            html_file=html_file
                        )
                    )
...
if __name__ == "__main__":
    ...
    if "-?" in args or "-h" in args or "--help" in args:
        printc(
            """<yellow>strapdown2html.py<white>: -h | [options] file1 [file2 [file3 ..]]
        ...
        try:
            ...
            with codecs.open(args[i], "r", encoding="utf-8") as file1:
                try:
                    ...
                    # printc("<ERROR> Exception found: <yellow>{e}<white>.".format(e=e))
                    printc(
                        "<WARNING> Failed to read title from the file '{file1}'.<white>".format(
                            file1=file1
                        )
                    )
        except:
